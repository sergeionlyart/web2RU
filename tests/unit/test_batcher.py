...
def test_build_batches_prefers_section_boundary_for_filled_batch() -> None:
    items = [
        TranslationItem(id=f"t_{idx}", text="a" * 190, block_id="b1", section_hint="b1")
        for idx in range(1, 9)
    ]
    items.extend(
        [
            TranslationItem(id="t_9", text="b" * 190, block_id="b2", section_hint="b2"),
            TranslationItem(id="t_10", text="b" * 190, block_id="b2", section_hint="b2"),
        ]
    )
    ...
    assert [item.id for item in with_boundary[0].items] == [f"t_{idx}" for idx in range(1, 9)]
    assert [item.id for item in with_boundary[1].items] == ["t_9", "t_10"]
