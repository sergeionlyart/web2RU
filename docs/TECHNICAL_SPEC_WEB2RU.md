...
* Перевод MUST не ломать inline‑элементы (`<span>`, `<strong>`, `<em>`, `<code>`, …) и ссылки.
...
* –drop-noscript auto|on|off — обработка `<noscript>` (по умолчанию auto; см. 8.5).

* –block-iframe auto|on|off — блокировать `<iframe>` (по умолчанию auto→on при freeze-js=on).
...
* inline code вида `--flag` (если встретилось как текст, но не внутри `<code>`),
...
1. **Нейтрализовать исполняемые `<script>`**

   * `<script src>`:
     * удалить `src`,
     * перенести исходное значение в `data-web2ru-src`,
     * поставить `type="application/x-web2ru-disabled"`.

   * inline `<script>` (без `src`):
...
* `<script type="application/json" ...>` (например `__NEXT_DATA__`), при условии что они не исполняются.

### **8.3**

### **freeze-js=off**

* Не трогаем `<script>`, не трогаем inline‑обработчики событий (`on*`).
...
* если freeze-js=on: удалять `<noscript>`, но предварительно:

  * если внутри `<noscript>` есть `img/link` (частый lazy‑fallback) — попытаться перенести в DOM (best effort),
